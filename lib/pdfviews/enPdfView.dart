...
import 'dart:convert';
class enPDFViewerScreen extends StatefulWidget {
  final File file;

  enPDFViewerScreen({required this.file});

  @override
  _enPDFViewerScreenState createState() => _enPDFViewerScreenState();
}

class _enPDFViewerScreenState extends State<enPDFViewerScreen> {
            ...
            Positioned(
              ...
              child: Column(
                children: [
                  ElevatedButton(
                    onPressed: () {
                      word = word.replaceAll(RegExp(r'[^АаӘәБбВвГгҒғДдЕеЁёЖжЗзИиЙйКкҚқЛлМмНнҢңОоӨөПпРрСсТтУуҰұҮүФфХхҺһЦцЧчШшЩщЪъЫыІіЬьЭэЮюЯяabcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ]'), '');
                      print(word);
                                  ...
                                  child: ListView(
                                    children: <Widget>[
                                      ListTile(
                                        ...
                                        onTap: () async {
                                          word = word.replaceAll(RegExp(r'[^A-Za-z]'), ''); // Убедитесь, что слово содержит только буквы
                                      ...
                                      ListTile(
                                        ...
                                        onTap: () async {
                                          try {
                                            final translatedText = await translateText(word, 'en', 'ru'); // Adjust source and target languages as needed
                                            showDialog(
                                              context: context,
